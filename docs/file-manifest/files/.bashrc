...
#======================================================================#
# =================== Commodore 64 Terminal Theme ==================== #
#======================================================================#

### Load the ANSI color variables (pointing to the locked .Xresources buckets)
# Normal Colors (0-7)
black='\[\e[0;30m\]'   # Color 0: Black
red='\[\e[0;31m\]'     # Color 1: Red
green='\[\e[0;32m\]'   # Color 2: Green
yellow='\[\e[0;33m\]'  # Color 3: Yellow
blue='\[\e[0;34m\]'    # Color 4: C64 Dark Blue
purple='\[\e[0;35m\]'  # Color 5: Purple
cyan='\[\e[0;36m\]'    # Color 6: Cyan
white='\[\e[0;37m\]'   # Color 7: C64 Light Blue (Default Text)

# Bright/Bold Colors (8-15)
darkgrey='\[\e[1;30m\]' # Color 8: Dark Grey
RED='\[\e[1;31m\]'      # Color 9: Light Red
GREEN='\[\e[1;32m\]'    # Color 10: Light Green
orange='\[\e[1;33m\]'   # Color 11: Orange
BLUE='\[\e[1;34m\]'     # Color 12: C64 Light Blue (Border)
grey='\[\e[1;35m\]'     # Color 13: Grey
lightgrey='\[\e[1;36m\]'# Color 14: Light Grey
purewhite='\[\e[1;37m\]'# Color 15: Pure White
...
# liviOS Loading System
load() {
    # If the user types 'load' without an argument
    if [ -z "$1" ]; then
	...
	# Execute all argument ($@), silence output and clear
	"$@" > /dev/null 2>&1 && clear

    else
        # Custom informative error
        echo "SEARCHING FOR ${1^^}"
        sleep 1
        echo "${1^^} NOT FOUND"
    fi
