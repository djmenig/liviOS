...
| `xgalaga-sdl` from OSS | Satisfied by the primary Leap 15.6 OSS repo (`--set-repo`); no separate `games` repo is declared (its Leap 15.6 branch was removed upstream) | `grep -in 'games' editions/opensuse/appliance.kiwi` (expect no match) |
